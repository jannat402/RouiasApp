        ...
        <tbody>
            @foreach($productos as $p)
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="border p-3">{{ $p->vendidos }}</td>
                    <td class="border p-3">
                        <a href="{{ route('admin.editar', $p->id) }}" class="text-blue-600 hover:underline">
                            Editar
                        </a>
                    </td>
                </tr>
...
<form action="{{ route('admin.descuento') }}" method="POST" class="flex gap-3 mb-8">
    @csrf
